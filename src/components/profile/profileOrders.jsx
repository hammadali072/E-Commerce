...
import {
    ...
    ClockIcon, CaretDownIcon, MapPinIcon, MagnifyingGlassIcon,
    ReceiptIcon, CreditCardIcon, ChatCircleTextIcon, DownloadSimpleIcon
...
const MOCK_ORDERS = [
    {
        ...
        products: [
            { name: 'Nike Air Max 2026', qty: 1, price: 180.00, img: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&q=80&w=100' },
            { name: 'Running Socks Pack', qty: 2, price: 34.50, img: 'https://images.unsplash.com/photo-1586350977771-b3b0abd50c82?auto=format&fit=crop&q=80&w=100' }
        ],
        billing: { subtotal: 234.00, shipping: 15.00, tax: 0.00 },
        shipping: '123 Fashion Ave, NY 10001',
        payment: 'Visa •••• 4242',
        timeline: [
    ...
    },
    {
        ...
        products: [
            { name: 'Luxury Watch', qty: 1, price: 125.50, img: 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&q=80&w=100' }
        ],
        billing: { subtotal: 125.50, shipping: 0.00, tax: 0.00 },
        shipping: '456 Trend St, LA 90001',
    ...
    },
    {
        ...
        products: [
            { name: 'Wireless Headphones', qty: 1, price: 69.00, img: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&q=80&w=100' },
            { name: 'Phone Case', qty: 1, price: 20.00, img: 'https://images.unsplash.com/photo-1586105251261-72a756657311?auto=format&fit=crop&q=80&w=100' }
        ],
        billing: { subtotal: 89.00, shipping: 0.00, tax: 0.00 },
        shipping: '123 Fashion Ave, NY 10001',
        payment: 'Apple Pay',
        timeline: [
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-5 border-b border-gray-100">
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="pl-9 pr-4 py-2.5 text-xs border border-gray-200 outline-none focus:border-amber duration-300 w-full sm:w-72 font-semibold text-dark bg-off-white"
                    />
                </div>
            </div>
                ...
                {filtered.map(order => {
                    ...
                    return (
                        <div key={order.id} className="border border-gray-100 bg-white hover:shadow-md duration-300">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap mt-4 sm:mt-0 ml-auto w-full sm:w-auto">
                                        {order.status !== 'Cancelled' && (
                                            <ThemeButton
                                                variant="outline"
                                                className="flex-1 sm:flex-none text-xs px-5 py-2.5 border tracking-widest font-semibold"
                            ...
                            {/* Expanded Details */}
                            {isOpen && (
                                <div className="border-t border-gray-100 p-6 sm:p-8 bg-white animate-fadeIn">
                                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">

                                        {/* Products & Summary (Left) */}
                                        <div className="lg:col-span-7 xl:col-span-8 flex flex-col gap-8">
                                            <div>
                                                <p className="text-xs font-bold text-dark/30 uppercase tracking-[0.2em] mb-5 flex items-center gap-2">
                                                    <PackageIcon size={16} weight="bold" />
                                                    Items Ordered ({order.products.length})
                                                </p>
                                                <div className="flex flex-col gap-4">
                                                    {order.products.map((p, i) => (
                                                        <div key={i} className="flex items-center gap-4 group p-2 -ml-2 hover:bg-light-bg duration-300">
                                                            <div className="size-14 bg-off-white border border-gray-100 flex-shrink-0">
                                                                <img src={p.img} alt={p.name} className="w-full h-full object-contain p-2" />
                                                            </div>
                                                            <div className="flex-1 min-w-0">
                                                                <h4 className="text-sm font-bold text-dark truncate group-hover:text-amber duration-300">{p.name}</h4>
                                                                <p className="text-xs text-dark/40 font-semibold mt-0.5">Quantity: {p.qty}</p>
                                                            </div>
                                                            <div className="text-right">
                                                                <p className="text-sm font-bold text-dark">${p.price.toFixed(2)}</p>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6 border-t border-gray-50">
                                                <div>
                                                    <p className="text-[10px] font-bold text-dark/30 uppercase tracking-[0.2em] mb-3">Shipping Address</p>
                                                    <div className="flex gap-3">
                                                        <MapPinIcon size={18} className="text-dark/20 flex-shrink-0" />
                                                        <p className="text-xs font-semibold text-dark/70 leading-relaxed">{order.shipping}</p>
                                                    </div>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] font-bold text-dark/30 uppercase tracking-[0.2em] mb-3">Payment Method</p>
                                                    <div className="flex gap-3">
                                                        <CreditCardIcon size={18} className="text-dark/20 flex-shrink-0" />
                                                        <p className="text-xs font-semibold text-dark/70 leading-relaxed">{order.payment}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="lg:col-span-5 xl:col-span-4">
                                            <div className="bg-light-bg p-6 border border-gray-100">
                                                <p className="text-xs font-bold text-dark/30 uppercase tracking-[0.2em] mb-5 flex items-center gap-2">
                                                ...
                                                <div className="flex flex-col gap-3 pb-4 border-b border-gray-200">
                                                    <div className="flex justify-between text-xs font-semibold">
                                                        ...
                                                        <span className="text-dark">${order.billing.subtotal.toFixed(2)}</span>
                                                    </div>
                                                    <div className="flex justify-between text-xs font-semibold">
                                                        <span className="text-dark/50">Shipping</span>
                                                        <span className="text-dark">{order.billing.shipping === 0 ? 'FREE' : `$${order.billing.shipping.toFixed(2)}`}</span>
                                                    </div>
                                                    <div className="flex justify-between text-xs font-semibold">
                                                        <span className="text-dark/50">Tax</span>
                                                        <span className="text-dark">${order.billing.tax.toFixed(2)}</span>
                                                    </div>
                                                </div>
                                                <div className="flex justify-between items-center pt-4 mb-8">
                                                    <span className="text-sm font-bold text-dark uppercase tracking-widest">Order Total</span>
                                                    <span className="text-xl font-playfairDisplay font-black text-dark">${order.total.toFixed(2)}</span>
                                                </div>

                                                <div className="flex flex-col gap-3">
                                                    <button className="flex items-center justify-center gap-2 w-full py-3 bg-dark text-white text-[10px] font-bold uppercase tracking-widest hover:bg-amber hover:text-dark duration-500">
                                                        <DownloadSimpleIcon size={14} weight="bold" />
                                                        Download Invoice
                                                    </button>
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <button className="flex items-center justify-center gap-2 py-3 border border-gray-200 text-[10px] font-bold uppercase tracking-widest hover:border-dark duration-300">
