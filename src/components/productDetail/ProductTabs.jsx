...
import { PRODUCT_TABS, productHighlights, shippingCards } from '../../Data';
                ...
                <div className="flex gap-4 md:gap-8 lg:gap-12 mb-8 md:mb-12 border-b border-gray-100 overflow-x-auto scrollbar-hide">
                    {PRODUCT_TABS.map((tab) => (
                        <button
                            ...
                            className={`pb-3 md:pb-4 text-[10px] sm:text-xs md:text-sm lg:text-base font-bold uppercase tracking-[0.15em] md:tracking-[0.2em] relative transition-all duration-300 whitespace-nowrap flex-shrink-0 ${
                    ...
                    {activeTab === 'Description' && (
                        <div className="animate-fadeIn">
                            <div className="max-w-4xl mb-10 md:mb-16">
                                <p className="text-dark/60 text-sm md:text-base lg:text-lg leading-relaxed mb-4 md:mb-6">
                                    Redefining contemporary style with a focus on heritage craftsmanship. This {product.name} is meticulously engineered for the modern individual who demands both performance and aesthetic excellence.
                                </p>
                                <p className="text-dark/60 text-sm md:text-base lg:text-lg leading-relaxed">
                                    Whether you're navigating the urban landscape or attending a high-stakes meeting, this piece offers the versatility and durability required for a demanding lifestyle. Made from premium materials sourced from the world's finest suppliers.
                                </p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 md:gap-8 lg:gap-12">
                                {productHighlights.map(({ icon: Icon, title, desc }) => (
                                    <div key={title} className="flex flex-col items-start">
                                        ...
                                        <p className="text-xs md:text-sm text-dark/40 leading-relaxed">{desc}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="animate-fadeIn w-full md:max-w-2xl">
                            <div className="flex flex-col border border-gray-100 divide-y divide-gray-100">
                                {staticSpecs.map(({ label, value }, idx) => (
                                    <div key={label} className={`flex p-3 md:p-4 ${idx % 2 === 0 ? 'bg-[#FAFAFA]' : 'bg-white'}`}>
                        ...
                        <div className="animate-fadeIn">
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
                                {shippingCards.map(({ icon: Icon, title, desc, badge }) => (
                                    <div key={title} className="flex flex-col p-5 md:p-8 bg-[#FAFAFA] border border-gray-100 transition-all duration-300 hover:bg-white hover:border-primary">
